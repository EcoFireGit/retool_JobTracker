...
let pts_per_QBO_Unit = Object.fromEntries(
  {{ table3.data }}.map(r => [r.QBO_Name, r.QBO_Points/(r.QBO_Target - r.QBO_Beginning_Value)]));

// for each QPI, store its delta value & sum of all points
let pts_QPIs = Object.fromEntries({{ table9.data }}.map(r => [r.QPI_Name, [r.QPI_Target - r.QPI_Beginning_Value,0]]));
...
let pts_Jobs = Object.fromEntries({{ table5.data }}.map(r => [r.Job_Title,0]));
